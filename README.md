...
購入物と Unity プロジェクトを照合し、「**導入済み / 未導入 / 一部だけ導入（％）**」を自動判定。昔入れたものも今から判定でき、**二重導入も警告**します。
...
Unity と lilToon / Poiyomi が手元にあれば、本物のシェーダで多角度プレビュー画像 + ブラウザで回せる 3D(GLB) を生成。**無くてもカタログ機能はそのまま使えます**。
