...
BOOTH などで買い集めた `.unitypackage`（アバター・衣装・小物・ギミック）を、**Unity で開かずに**棚卸し・検索・プレビューし、新しい商品を**取り込む前に「アバターがピンクになる事故」を止めます**。ローカルで読み取り専用に動く非公式ツールです（表示用に公開BOOTHのサムネ・商品情報のみ取得。Windows 専用・インストール不要）。
...
> **VRChat 非公式ツール** ／ **ローカルで読み取り専用**。購入ファイルのアップロード・自動ログイン・購入ライブラリの一括取得・ファイル改変はしません。表示用に **公開BOOTHのサムネ・商品情報のみ** 取得します（ログイン不要）。

[PRIVACY.md](PRIVACY.md) を参照。要点:

- **あなたのデータは送信しません**（ライブラリ・購入ファイル・利用状況をアップロードしない）。表示用に **公開BOOTHのサムネ・商品情報** と、3Dビューアの three.js（CDN）を取得します。
- **読み取り専用**。購入物の**変換も再配布もしません**。**自動ログイン・購入履歴の一括取得・自動ダウンロードはしません**。
...
技術的な要は **GUID 突合**（プロジェクトの `.meta` GUID × 購入物の GUID）。これにより導入の遡及検出・取り込み前の競合 diff・重複の同定を、すべて読み取り専用・ローカル処理で実現しています（表示用の BOOTH サムネ・商品情報のみ別途取得）。
