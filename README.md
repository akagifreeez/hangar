...
**▶ ダウンロード:** [最新リリース](https://github.com/akagifreeez/hangar/releases/latest) から `Hangar-…-portable-win-x64.zip` を取得 → 展開して **`Hangar.cmd`** から起動（Windows 専用・インストール不要）。
...
- **カタログ化** — フォルダを指定すると中の `.unitypackage` を**展開せず**に解析し、サムネ付きグリッドで一覧。中身（ファイル構成・プレビュー画像）も閲覧。並列解析＋差分スキャンで、2回目以降の再スキャンは変更分だけを見て**一瞬**で終わります。`.unitypackage` やフォルダを**ウィンドウにドラッグ&ドロップ**しても取り込めます。
- **種別の自動分類** — 各パッケージを中身のシグナル（メッシュ/テクスチャ vs シェーダ/スクリプト/エディタ拡張）から **3Dモデル / ツール / アニメ / マテリアル** に自動分類し、カタログをタブで絞り込み。lilToon・Poiyomi・SDK などの**ツールが棚に埋もれず**、3D生成（🎬）は3Dモデルにだけ表示されます。
- **取り込み前チェック（インポート競合 diff）** — `.unitypackage` を取り込む**前**に、対象 Unity プロジェクトと GUID 突合して「**何を上書きするか／足りないシェーダはないか**」を表示。**アバターがピンクになる取り込み事故**（lilToon / Poiyomi の二重定義・バージョン降格、必要シェーダの欠落）を未然に防ぎます。「既に導入済み商品の再取込」は誤警告しません。GUI の「📦 取り込み前チェック」、または `diff` コマンド（共有用 HTML レポートも出力可）。
...
- **再現テンプレ（save / restore）** — 改変済みアバターのプロジェクトから「**自分が作ったファイルだけ**」を保存し、「**どの購入物に依存しているか**」をマニフェスト化（購入物のバイトは**一切同梱しません**＝GUID で名指すだけ）。まっさらなプロジェクトに復元すると「この購入物は**持ってる/入れ直して**」を案内。別PC・新バリエーションの土台づくりに。読み取り専用・外部通信なし・自動DLなし（復元は既定で既存ファイルを保護）。
...
1. `Hangar.cmd` を実行（上記「ダウンロードと初回起動」を参照）。
...
5. 新しい `.unitypackage` を取り込む前に「📦 取り込み前チェック」で、その商品が既存プロジェクトの**何を上書きするか／足りないシェーダはないか**を確認できます。
...
| `cli.ts` | scan / list / search / detect / diff / save-template / restore-template / installs / catalog / dupes / products / reclaim / render / caps / version |
